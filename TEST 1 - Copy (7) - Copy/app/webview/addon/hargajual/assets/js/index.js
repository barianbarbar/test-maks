...
function selectMenu() {
  ...
  var nama_operator = ["x"];
  // console.log(objMenu[menudipilih].produk);
  for (let i = 0; i < objMenu[menudipilih].produk.length; i++) {
    ...
    if (catatan == "PPOB" || catatan == "TIKET KAI") {
...
function myfunc() {
  ...
  const saveData = {
    kode: final_kode_produk,
    admin: catatan === "PPOB" || catatan === "TIKET KAI" ? inputTotal.value : inputKeuntungan.value,
    perintah: perintah,
    aksi: "post",
  };
